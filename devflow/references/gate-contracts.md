...
- **结构图七类清单（v3.29.3）**：除时序图外，详设应在冻结清单中登记结构图（不参与 SEQ/WHEN
  计数，纯可读性增益），推荐七类及其归位原则（图随所属内容走）：
  ① 状态机图 stateDiagram-v2 → 紧随 §2.3 状态转移表（用户多轴复合一张 + 组织一张）；
  ② 权限/核心机制模型图 flowchart → §4 权限矩阵后（并集计算等最难点）；
  ③ 决策链图 flowchart → 对应规则域 §5.x 节首（登录校验闸门序/三层排序回退）；
  ④ 对象生命周期 stateDiagram → 所属 BOP 小节内（如回收站条目）；
  ⑤ 页面导航图 flowchart → §7.1 页面清单表后，**从 PAGES 数据派生**；
  ⑥ ER 图 → §2.1（既有惯例）；
  ⑦ 全文档图表总清单声明 → §6.0 仅声明（时序 N 张分布 + 结构图 M 类分布），不承载图。
- **数据库中立表述（v3.29.3）**：详设正文与图中**不出现具体数据库产品名**（如 H2/Oracle），
  统一以 DB 泛称（图参与者 `participant DB`；正文"DB 文件模式/方言由部署配置"）；具体方言
  记载归《技术选型》《数据库设计决策》与部署配置，详设保持可迁移中立。
- **固定流水线顺序不可省步（v3.29.3 实测）**：`gen_<feature>.py 重写全文 → df_pipeline design
  重拼接 df 块 → s2 gate`。跳过中间渲染步会因 df:begin 块内容回退触发 s2 大面积假失败
  （实测 246 处），补渲染即归零——生成器重写后必须重跑 df_pipeline 再进 gate。
