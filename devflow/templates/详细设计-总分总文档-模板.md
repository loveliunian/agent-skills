...
【产物契约——机器校验，违者 Gate FAIL】
 ...
 2. 追溯矩阵在独立文档 `docs/详细设计/<feature>-需求追溯.md`（正本锚点 `<!-- anchor: acceptance-traceability -->`，v3.28.1 移出详设，管线 `--trace-doc` 渲染）；本文档只收 §8 验收标准，逐点落点由追溯矩阵承担。实现交接在 `<feature>-实现交接.md`。
...
> **结构化渲染块（v3.24.0/A07；v3.28.1 块分角色）**：本文含 9 个 `<!-- df:begin:KEY -->` 锚点块，由
...
> **业务规则分布（v3.28.1）**：模块页面规则在各分文档 §7.2 页组「业务规则」表；全局/无页面规则在本文 §4.1–§4.2；上表为全量索引。
...
> 本节索引为章节级并集，不设 N.M.K 操作小节；模块内操作明细在分文档按三层编号展开——
> monolith `#### 6.2.N` / 总分分 `#### 4.2.N`，且必须挂 `### X.2 业务操作契约（BOP-1～BOP-N）`
> 父级标题（X=6/4；缺父级 df_validate L-HIER-1 拦截）。
...
> `docs/详细设计/<feature>-需求追溯.md`（v3.28.1 移出详设）。
