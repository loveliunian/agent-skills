...
【产物契约——机器校验，违者 Gate FAIL】
 ...
 2. 追溯矩阵在各分文档（正本锚点 `<!-- anchor: acceptance-traceability -->`）；本文档只收 §9 验收标准与跨模块覆盖率基线，逐点落点由分文档矩阵承担，编码前用 §9 验收标准做分母对账。
...
10. 错误码三层引用不重定义：平台通用码（本文 §5 接口规范）→ 业务码（分文档规则/接口定义）→ 前端行为（分文档 §7.6）。写契约不写实现快照。
【Gate 环境实操】
...
（由 design.json 自动生成 8 列追溯矩阵：ID|PRD 锚点|页面/任务|接口|数据|规则|测试用例|状态——跨模块全局口径；逐点明细由分文档矩阵承担）
<!-- df:end:trace-matrix -->
...
> 总文档登记跨模块影响面索引；每个模块的文件级交接表在对应分文档 §15。
> 基线来自 P2 的现有代码基线调查（见 `phases/02-详细设计.md`）。

| 模块 | 分文档 | 主要改动文件/符号 | ADD/MODIFY/DELETE | 验收点 |
|------|--------|-------------------|-------------------|--------|
| {M-01 模块名} | `M-01-{ModuleName}-详细设计.md` | 见分文档 §15 实现交接 | 见分文档 §15 | M-{xx}-F{y}-A{z} |
