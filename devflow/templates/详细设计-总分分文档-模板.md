...
【写作规则——评审真正在意的】
 ...
 8. 表结构全字段五列，禁止"关键字段"式省略（§2.3）；每个关键流程 WHEN 伪代码+时序图成对（§4；业务操作小节 `#### 4.2.N` 必须挂在 `### 4.2 业务操作契约（BOP-1～BOP-N）` 父级标题下，缺父级 df_validate L-HIER-1 拦截）；每个接口"请求+响应"两张六列表（§5.3），禁止"契约同 §x.x"式省略。
...
>
> 小节标题须带 BOP 编号且挂在 §4.2 父级下（`### 4.2 业务操作契约（BOP-1～BOP-N）` → `#### 4.2.N 操作名（BOP-n）`，如 `#### 4.2.1 新增流程（BOP-1）`），操作表关联列的 BOP-n 按此定位；
> 缺父级标题即层级断裂，df_validate 标题层级校验拦截（L-HIER-1）；§4.1 预留给跨操作时序（如级联/到期），无则不写本小节；
> 操作以业务命名（非 CRUD 枚举），覆盖触发者、前置校验、源/目标状态、事务与并发、结果与失败、
> 关联对象处置；冻结验收点的每项操作都必须有落点（Gate 拦截覆盖缺口）。

### 4.2 业务操作契约（BOP-1～BOP-N）
...
| 操作 | 类型 | 权限点 | 关联（§5.3.x · BOP-n · Rn） | 触发弹窗/抽屉 |
|------|------|------|------|------|
| {删除} | 行操作 | {module}:delete | §5.3.x · BOP-n · R{n} | → 弹窗/抽屉：{删除确认} |
| {批量导出} | 批量操作 | {module}:export | §5.3.x · BOP-n · R{n} | — |
...
| 交互 | 组件 | 接口（§5.3.x） | 关键状态/确认流 |
|------|------|------|------|
| {新增/编辑} | views/{module}/EditDialog.vue | §5.3.x POST/PUT | {可填写；字段校验/失败保留输入} |
| {删除确认} | views/{module}/DeleteConfirmDialog.vue | §5.3.x DELETE | {二次确认文案/409 引用拦截跳转} |
...
| 字段 | 标签 | 控件 | 校验与提示 | 候选来源 | 链路（§5.3.x → 表.字段） |
|------|------|------|------|------|------|
| {field} | {标签} | {控件} | {必填 · 长度/范围 · 格式 · 唯一性/条件显隐/只读 · R{n}} | {候选来源} | {§5.3.x → 表.字段 或 —} |
