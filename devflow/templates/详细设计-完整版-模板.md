...
12. 占位符检查是子串匹配
12. 占位符检查是子串匹配
13. **锚点具体化（v3.26.10 · L-P2-004，七问题复盘）**：① §2.2 每张表独立编号 `#### 2.2.N 表名`，table-index 锚点列=具体表锚点；② §3.1 概览表首列=详细定义（§3.2.N），无概览锚点列；③ §3.2 每节标题只写 `#### 3.2.N 名称`，方法/路径/权限写标题下首行 `> 说明：METHOD 路径｜权限：…`（③⑥⑨ 均已硬校验）；④ §6 业务操作表首列=§锚点，操作小节编号 `#### 6.2.N`；⑤ §7.1 每个页面独立编号小节（§7.1.N）；⑥ §7.3 页组必须覆盖 §7.1 全部页面（§7.3.N 小节数 ≥ 页面数，硬校验）；⑦ §11.1 追溯矩阵页面/接口/数据三列必须写具体锚点（§7.1.N/§3.2.N/§2.2.N），禁止写 §7.1/§3.1/§2.2 通用锚点——df_validate 按 L-P2-004 拦截共用锚点与 anchor≠detail_anchor；⑧ §4 权限矩阵拆页面/接口两块，接口块必含"接口说明"列（=§3.2 详细定义名称）；⑨ §5 规则表锚点列前置且每条规则锚点落到所属小节（WHEN 行逐字注入锚点小节；规则锚点单一化即 FAIL）（TODO/TBD/待补充/REPLACE_WITH/占位/暂定，大小写不敏感）：领域词撞车改英文（占位符→placeholder、待办计数→backlog）；`{{...}}` 模板变量未替换同样视为未完成。
...
<!-- df:begin:api-index -->
（由 design.json 自动生成接口概览：详细定义|方法|路径|接口名称|权限|请求字段|响应字段——v3.26.10 L-P2-004 列序，无概览锚点列）
<!-- df:end:api-index -->
...
| 详细定义 | 方法 | 路径 | 接口名称 | 权限 |
|------|------|------|----------|------|
| §3.2.1 | GET | /api/{feature}/page | 分页查询 | {feature}:view |
...
（由 design.json 自动生成两块：①页面/菜单矩阵 §锚点|页面/菜单|所需权限；②接口矩阵 §锚点|方法|路径|接口说明|所需权限——接口说明=§3.2 详细定义名称）
...
<!-- df:begin:rule-index -->
（由 design.json 自动生成规则索引：§锚点|规则|摘要——v3.27.1 L-P2-004 锚点列前置）
<!-- df:end:rule-index -->
...
<!-- df:begin:biz-ops -->
（由 design.json 自动生成业务操作契约索引：§锚点|操作|触发者/触发|源状态→目标状态|事务/并发|结果/失败|验收点——v3.26.10 L-P2-004 锚点列前置）
<!-- df:end:biz-ops -->
