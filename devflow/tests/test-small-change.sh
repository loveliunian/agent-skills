...
# v3.28.4(P0-1)：released 需要发布授权收据
mkdir -p "$W/.devflow/order-filter/authorizations"
printf '{"feature":"order-filter","target":"production","authorized_by":"user","authorization_source":"explicit-user-request","authorized_at":"%s","scope":["deploy"]}\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$W/.devflow/order-filter/authorizations/release.json"
