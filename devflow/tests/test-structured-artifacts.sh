...
check_rc 0 "sample design validates" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --criteria criteria.md
check_rc 0 "sample design pipeline renders into skeleton" "${DEVFLOW_PY[@]}" "$P" design --input design.json --doc doc.md --criteria criteria.md --db-doc dbdoc.md --trace-doc tracedoc.md
...
check_rc 1 "missing anchor block fails closed (render)" "${DEVFLOW_PY[@]}" "$P" design --input design.json --doc partial.md --criteria criteria.md
...
PYEOF
check_rc 1 "orphan table rejected without unreferenced_reason" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-orphan.json --criteria criteria.md
check_rc 0 "orphan accepted with unreferenced_reason" bash -c "\$DEVFLOW_PY_STR -c \"import json;d=json.load(open('d-orphan.json'));d['tables'][-1]['unreferenced_reason']='共享日志表';d['tables'][-1]['fields']=[{'name':'id','type':'BIGINT','constraint':'PK','default':'-','note':'x','ddr':['DDR-1']}];json.dump(d,open('d-orphan2.json','w'),ensure_ascii=False)\" && \$DEVFLOW_PY_STR '$V' --kind design --input d-orphan2.json --criteria criteria.md"
check_rc 1 "broken anchor reference rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-broken-ref.json --criteria criteria.md
check_rc 1 "undeclared empty collection rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-zero.json --criteria criteria.md
check_rc 1 "placeholder wording rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-ph.json --criteria criteria.md
check_rc 1 "criteria set inequality rejected" bash -c "printf '| M01-F01-A01 |\n' > few.md && \$DEVFLOW_PY_STR '$V' --kind design --input design.json --criteria few.md"
...
check_rc 0 "design validates with --doc (overview↔detail closure)" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --criteria criteria.md --doc doc.md
# 反向：概览有、详细定义缺
grep -v "3.2.2 创建退款单" doc.md > doc-missing.md
check_rc 1 "overview API without detail section rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --doc doc-missing.md
# 反向：文档多余小节未被概览收录
"${DEVFLOW_PY[@]}" - <<'PYEOF'
from pathlib import Path
...
PYEOF
check_rc 1 "detail section missing from overview rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --doc doc-extra.md
# 反向：detail_anchor 重复
"${DEVFLOW_PY[@]}" - <<'PYEOF'
import json
...
PYEOF
check_rc 1 "duplicate detail_anchor rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-dup-anchor.json --doc doc.md
...
PYEOF
check_rc 1 "field without DDR reference rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-noddr.json --doc doc.md
check_rc 1 "dangling DDR reference rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-dangling.json --doc doc.md
check_rc 1 "orphan DDR without waiver rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-orphan-ddr.json --doc doc.md
check_rc 0 "orphan DDR with unreferenced_reason accepted" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-orphan-ok.json --doc doc.md
...
PYEOF
check_rc 1 "page missing route rejected (required)" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-noroute.json --criteria criteria.md
check_rc 1 "dialog api anchor dangling rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-dangling-dialog.json --criteria criteria.md
check_rc 1 "table_columns field not in doc rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-doc-drift.json --criteria criteria.md --doc doc.md
check_rc 1 "dialog component not in doc row rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-dialog-drift.json --criteria criteria.md --doc doc.md
...
PYEOF
check_rc 0 "page element → API → table chain resolves, doc row matches" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-chain-ok.json --criteria criteria.md --doc doc-chain.md
CH_API=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-chain-badapi.json --criteria criteria.md 2>&1 || true)
printf '%s' "$CH_API" | grep -q "链路断链" \
  && ok "dangling api_field anchor rejected (v3.28.1)" || bad "api_field closure missed: $CH_API"
CH_SRC=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-chain-badsrc.json --criteria criteria.md 2>&1 || true)
printf '%s' "$CH_SRC" | grep -q "落库字段必须真实存在" \
  && ok "dangling source table field rejected (v3.28.1)" || bad "source closure missed: $CH_SRC"
check_rc 1 "dangling form submit_api rejected (v3.28.1)" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-chain-badform.json --criteria criteria.md
...
PYEOF
NDR_OUT=$("${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --criteria criteria.md --doc doc-nodialogrow.md 2>&1 || true)
...
PYEOF
RWF_OUT=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-reserved-fail.json --criteria criteria.md 2>&1 || true)
...
RWW_OUT=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-reserved-warn.json --criteria criteria.md 2>&1 || true)
...
PYEOF
check_rc 1 "baseline.related_acceptance dangling rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-relacc.json --criteria criteria.md
check_rc 1 "business_operations empty test_scenarios rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-noscen.json --criteria criteria.md
...
PYEOF
EC_DUP=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-ec-dup.json --criteria criteria.md 2>&1 || true)
...
check_rc 1 "lowercase error code rejected by schema pattern" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-ec-format.json --criteria criteria.md
EC_DOC=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-ec-docmiss.json --criteria criteria.md --doc doc.md 2>&1 || true)
...
check_rc 0 "execution-plan sample validates" "${DEVFLOW_PY[@]}" "$V" --kind execution-plan --input ep.json
...
PYEOF
EP_FMT=$("${DEVFLOW_PY[@]}" "$V" --kind execution-plan --input ep-ref-format.json 2>&1 || true)
printf '%s' "$EP_FMT" | grep -q "格式非法" \
  && ok "execution-plan malformed design_ref rejected (v3.28.1)" || bad "design_ref format missed: $EP_FMT"
EP_UNK=$("${DEVFLOW_PY[@]}" "$V" --kind execution-plan --input ep-ref-unknown.json 2>&1 || true)
...
PYEOF
check_rc 0 "rules sharing a page-group anchor accepted (v3.28.1)" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-rules-page.json --criteria criteria.md --doc doc.md
RGEN=$("${DEVFLOW_PY[@]}" "$V" --kind design --input d-rules-generic.json --criteria criteria.md --doc doc.md 2>&1 || true)
...
PYEOF
check_rc 1 "execution-plan slice task_id dangling rejected" "${DEVFLOW_PY[@]}" "$V" --kind execution-plan --input ep-bad.json
check_rc 1 "execution-plan empty component rejected" "${DEVFLOW_PY[@]}" "$V" --kind execution-plan --input ep-bad2.json
...
"${DEVFLOW_PY[@]}" - "$SHA" <<'PYEOF'
import json, sys
d = json.load(open("verification.json"))
d["evidence"]["unit"]["report_sha256"] = sys.argv[1]
json.dump(d, open("verification.json", "w"), ensure_ascii=False)
PYEOF
check_rc 0 "sample verification validates with baseline + exec record" "${DEVFLOW_PY[@]}" "$V" --kind verification --input verification.json --baseline baseline.tsv --exec-record exec.env
check_rc 0 "verification pipeline renders report" "${DEVFLOW_PY[@]}" "$P" verification --input verification.json --out report.md --baseline baseline.tsv --exec-record exec.env
...
check_rc 1 "FAIL row rejected (FAIL=0 hard rule)" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-fail.json --baseline baseline.tsv
check_rc 1 "baseline set inequality rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-less.json --baseline baseline.tsv
check_rc 1 "tampered report SHA rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-sha.json --baseline baseline.tsv
check_rc 1 "placeholder cmd rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-cmd.json --baseline baseline.tsv
check_rc 1 "shell wrapper cmd rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-wrap.json --baseline baseline.tsv
check_rc 1 "shared report file rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-dup.json --baseline baseline.tsv
check_rc 1 "declared exit != gate actual rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-exit.json --baseline baseline.tsv --exec-record exec-bad.env
check_rc 1 "client waiver with wrong scope rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-cna.json --baseline baseline.tsv
...
for v in v-true-and v-colon v-ls v-bash-anywhere; do
  check_rc 1 "placeholder variant rejected: $v" "${DEVFLOW_PY[@]}" "$V" --kind verification --input "$v.json" --baseline baseline.tsv
done
printf 'UNIT_CMD=pytest -q\nUNIT_ACTUAL_EXIT=0\nINTEGRATION_CMD=mvn verify\nINTEGRATION_ACTUAL_EXIT=0\nCLIENT_CMD=npm run test:e2e\nCLIENT_ACTUAL_EXIT=0\nLOAD_CMD=UNKNOWN\nLOAD_ACTUAL_EXIT=0\nSTAGING_CMD=./scripts/smoke.sh staging\nSTAGING_ACTUAL_EXIT=0\n' > exec-cmd-mismatch.env
check_rc 1 "declared cmd != gate cmd rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input verification.json --baseline baseline.tsv --exec-record exec-cmd-mismatch.env
# CLIENT_EXEMPT 双向一致
"${DEVFLOW_PY[@]}" - <<'PYEOF'
import json
d = json.load(open("verification.json"))
...
PYEOF
printf 'UNIT_CMD=pytest -q\nUNIT_ACTUAL_EXIT=0\nINTEGRATION_CMD=mvn verify\nINTEGRATION_ACTUAL_EXIT=0\nLOAD_CMD=jmeter -n -t plan.jmx\nLOAD_ACTUAL_EXIT=0\nSTAGING_CMD=./scripts/smoke.sh staging\nSTAGING_ACTUAL_EXIT=0\nCLIENT_EXEMPT=1\n' > exec-exempt.env
check_rc 0 "CLIENT_EXEMPT=1 with matching declaration accepted" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-exempt-ok.json --baseline baseline.tsv --exec-record exec-exempt.env
check_rc 1 "CLIENT_EXEMPT=1 with contradicting declaration rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-exempt-bad.json --baseline baseline.tsv --exec-record exec-exempt.env
# M1: 非空集合的伪造零结果声明
"${DEVFLOW_PY[@]}" - <<'PYEOF'
import json
...
PYEOF
check_rc 1 "forged zero-result declaration on non-empty collection rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-forged-zero.json --doc doc.md
# L7: 重复声明
"${DEVFLOW_PY[@]}" - <<'PYEOF'
import json
...
PYEOF
check_rc 1 "duplicate zero-result path rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-dup-zero.json --doc doc.md
...
EOF
check_rc 0 "backend-only feature with declared-empty collections accepted" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-backend.json --doc doc.md
...
PYEOF
check_rc 0 "CJK no-space heading and fenced pseudo-heading accepted (M3)" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --doc doc-m3.md
# L5: pipeline 空 --gate 报用法错误
check_rc 2 "pipeline empty --gate rejected" "${DEVFLOW_PY[@]}" "$P" design --input design.json --doc doc.md --criteria criteria.md --gate

# 5c. v3.19.0 七项修复回归
# (1) pipeline 无 baseline/exec-record → 拒（曾渲染假「可以部署」）
check_rc 2 "pipeline without baseline/exec-record rejected" "${DEVFLOW_PY[@]}" "$P" verification --input verification.json --out report-x.md
...
PYEOF
check_rc 1 "empty report files rejected by content check" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-empty.json --baseline baseline.tsv --exec-record exec.env
...
PYEOF
check_rc 1 "forged verification zero-result rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-forged-zero.json --baseline baseline.tsv --exec-record exec.env
...
CLIENT_EXEMPT=1
' > exec-frozen.env
check_rc 1 "CLIENT_EXEMPT against frozen pc-web scope rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-frozen-mismatch.json --baseline baseline.tsv --exec-record exec-frozen.env --frontend-scope pc-web
check_rc 0 "CLIENT_EXEMPT with frozen not-applicable accepted" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-exempt-ok.json --baseline baseline.tsv --exec-record exec-exempt.env --frontend-scope not-applicable
...
PYEOF
check_rc 1 "table anchor missing from doc rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --doc doc-noanchor.md
check_rc 0 "all sample anchors resolve in doc" "${DEVFLOW_PY[@]}" "$V" --kind design --input design.json --doc doc.md
...
PYEOF
check_rc 1 "duplicate verification zero-result rejected" "${DEVFLOW_PY[@]}" "$V" --kind verification --input v-dup-zero.json --baseline baseline.tsv --exec-record exec.env
...
PYEOF
check_rc 1 "resources without operations rejected (compensation chain missing)" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-noops.json
check_rc 1 "reverse operation missing resource stance rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-noclosure.json
check_rc 1 "never release without evidence rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-never.json
check_rc 1 "integration without endpoint rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-noend.json
check_rc 1 "dead-only config + low confidence without reason rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-dead.json
check_rc 1 "undeclared empty optional collections rejected" "${DEVFLOW_PY[@]}" "$V" --kind design --input d-nodecl.json

# 6. 失败关闭：校验失败 → 不渲染
rm -f report2.md
check_rc 1 "pipeline aborts on invalid verification JSON" "${DEVFLOW_PY[@]}" "$P" verification --input v-fail.json --out report2.md --baseline baseline.tsv --exec-record exec.env
