...
- `small-change` 先加载 `commands/small-change.md` 扫描项目并分类；结果为 FULL 时自动回到 `change`，不得继续快速路径。`field-change` 是此模式的兼容别名。
...
| P1 | `s1_fact_sources_gate.sh docs/detailed-design`（校验技术选型机读绑定 + 约束文件 SHA 与 state 一致） |
