...
**测试锚点**：详设冻结的全部可交互控件——表单控件、操作按钮（design.json `pages[].form_controls/dialogs/actions[].test_anchor`）、弹窗/抽屉——实现时必须挂 `data-testid="<test_anchor>"` 属性，锚点值逐字照抄详设，禁止自造/改写；详设未冻结锚点的新增控件先回 P2 补冻结再实现。
