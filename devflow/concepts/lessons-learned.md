...
## L-详设-时序图（v3.29.1，m01-base）

**教训**：详设的 Mermaid 时序图由 LLM 会话内手写时逐轮不同（参与者命名/箭头文本/选例），且为凑
`SEQ>=WHEN` 门禁会产生零信息增量的填充图。

**对策**：
1. 归属与数量在 P2 前冻结：按业务操作（BOP）一操作一张，置于该 BOP 专属小节；规则级 WHEN 不单独配图。
2. 全部图由生成器脚本（tools/gen_<feature>.py）从 design 正本确定性渲染，重渲染逐字节稳定，禁止手写。
3. 填充图反模式禁止： WHEN 语义合并进所属 BOP 图，或在规则行标注"时序见 BOP-x 图"。
