...
### L-P2-004：详设锚点通用化七问题（m01-foundation P2a 复盘，v3.26.10）

**教训**：详设的确定性层（表索引/接口概览/业务操作表/追溯矩阵）若允许通用锚点（§2.2/§3.1/§7.1）与冗余列，读者无法从矩阵定位到具体条目——m01-foundation 项目 P2a 后用户复核发现七类问题，返工渲染器+校验器+三模板。

**七问题清单**：①表无编号（2.2.N）且表索引锚点非具体表；②接口概览含"概览锚点"冗余列、详细定义未前置；③§3.2 标题塞方法/路径/权限；④业务操作表锚点列在末位；⑤操作小节编号缺 6.2.x；⑥§7.2 页组未覆盖全部页面；⑦追溯矩阵页面/接口/数据三列写通用锚点，与上文一一对应断裂。

**预防（已固化）**：
1. `df_render.py`：api-index 列序=详细定义|方法|路径|接口名称|权限|请求字段|响应字段（无概览锚点列）；biz-ops 首列=§锚点。
2. `df_validate.py`（design）：pages/tables 锚点共用即 FAIL；apis anchor≠detail_anchor 即 FAIL（错误信息带 L-P2-004 与修复提示）。
3. 三份详设模板：渲染块描述、§3.1 样例表头、表样例编号、写作铁律第 13 条同步更新。
4. `phases/02-详细设计.md`：锚点具体化条款进"必须解决的设计问题"。
5. `design.sample.json`：tables 锚点 §2.2.N、apis anchor=detail_anchor、验收行三列具体化。
6. `df_render.py`：§4 权限矩阵拆页面/接口两块，接口块含"接口说明"列（=§3.2 详细定义名称）；§5 规则索引锚点列前置。
7. 规则锚点落到所属小节时，生成器必须把 WHEN 行逐字注入锚点小节（df_validate 的 when_line 同源校验按锚点小节查重）——只改锚点不注入会直接 FAIL。
8. 三处引导已升级为硬校验（check_design_doc_specificity）：§7.2.N 小节数 < 页面数 FAIL、规则锚点单一化 FAIL、§3.2 详细定义小节缺「> 说明：METHOD 路径｜权限」首行 FAIL——负向用例三条均已回归验证。

**适用阶段**：P2 详设产出、P2a 评审核查（评审员按 §11.1 抽三行验锚点可定位性）。

### L-EFF-001：devflow 运行时摩擦治理（v3.27.1，m01-foundation 全程复盘）

**教训**：从 PRD 到 P2a 全程约 25-30% 时长耗在 skill 内部契约摩擦（schema/Gate/渲染器三方不同源、占位话术子串误伤、权限码格式静默忽略、证明协议无引导、BSD grep 多字节退化）。摩擦不是质量，是返工。

**已固化**：见 v3.27.1 CHANGELOG 六项。执行策略侧两条同样有效：①业务裁决在启动前一次性给出；②设计修订攒批后只走一轮评审收据。

---

## L-M01 系列：m01-foundation 全流程实战教训（v3.27.5，2026-09-18）

### L-M01-001：MP 全局逻辑删除 × 回收站生命周期字段冲突（P0 级）

**教训**：`logic-delete-field` 全局配置使 MP 将该字段从一切 UPDATE SET 子句过滤（含 LambdaUpdateWrapper.set）——回收站业务的 deleted/recycle_ref 显式置位（删除进站/恢复/解锁）全链路静默不落库。

**规约**：逻辑删除字段参与业务状态机（回收站/生命周期唯一键）的服务**禁用**全局 logic-delete 配置；deleted 过滤在服务层显式表达；置位一律 LambdaUpdateWrapper.set 并配"删除→恢复→彻底删除"回环测试。

### L-M01-002：脚手架发行必须过"干净环境全量编译"冒烟

**教训**：code_base 发行版 framework-data 模块只有 target/classes 无 java 源码，全仓 mvn compile 失败；ProTable 存在既有 TS 错误。问题到 P3 编译期才暴露。

**规约**：P1 技术选型盘点必须执行 `mvn compile + pnpm typecheck` 双冒烟并留存输出；提供方修复发行流程。

### L-M01-003：实体字段禁止 FQCN 类型名

**教训**：`private java.time.LocalDateTime x;` 触发 P4b 实体字段扫描正则失配——datetime 字段集体报"Field missing"。

**规约**：实体一律短类型名 + import；P4b 扫描正则同时作为实体生成器的约束来源。

### L-M01-004：MP updateById 的空值陷阱

**教训**：updateById/Wrapper.set 对逻辑删除相关字段和 null 值默认跳过——`lock_until=NULL`（R50 解锁）、`recycle_ref=NULL`（R70 复位）静默不生效。

**规约**：凡"置空"语义的持久化一律 `LambdaUpdateWrapper.set(field, null)` 显式表达，并配"置空后回读断言"测试。

### L-M01-005：df_pipeline design --out 整篇覆盖销毁手写章节（P0 级）

**教训**：design 渲染对已存在的 4700 行手写详设执行骨架整篇重建，§2.2-§15 手写内容全部丢失（v1.1 重建事故，约 2h 返工）。

**已固化**：df_render.py v3.27.5 守卫——`--out` 目标含锚点块外内容即拒绝，指引改用 `--doc` 拼接模式。

### L-M01-006：p3_security_perf 三模式的共享报告收据死循环

**教训**：full 模式重渲染共享报告 → 先签发的 P3c/P3d 子收据证据树必然失配 → audit-receipts 误报 broken。

**已固化**：v3.27.5 起 full 模式签发 P3cd 后自动删除被取代的 P3c/P3d 子收据（含镜像）。

### L-M01-007：p4/p4b 共用 evidence 文件的格式冲突

**教训**：p4_validation 要求 2 列（ID/STATUS），p4_prd_vs_code 要求 4 列（含路径）——同一文件两门标准不同，实测三改格式。

**已固化**：v3.27.5 起 p4_validation 同时接受两种表头（状态列取 $NF）。

### L-M01-008：skill 中途升级的收据链全量重刷

**教训**：执行中途 skill 3.27.2→3.27.4 三次升级，audit-receipts "全链同版本"检查迫使全部 gate 重跑 3 轮（占比最大单项损耗）。

**已固化**：v3.27.5 起 audit 改判"链内混用"——全链同版本（任意版本）仅 WARN，链内 ≥2 个版本才 FAIL。

### L-M01-009：事实源骨架占位符逃过凭证门

**教训**：`_环境与账号.md` 含 `{dialect}`/`{component}` 未填占位符，P6 凭证门未扫描该文件——铁律 6 第一入口事实源不完整却全链放行。

**已固化**：v3.27.5 起 p6_credential_gate 增加 `_环境与账号.md` 占位符扫描（含未填即 exit 1）。

### L-M01-010：接口文档不得引用详设代替正文

**教训**：P9 接口文档只写端点速查行+"完整契约见详设 §3.2"，不满足消费方视角（缺请求/响应示例、错误码列举、curl 样例）；且 springdoc 依赖未引入导致 swagger 配置为死配置。

**规约**：P9 接口文档必须独立成文（每接口含示例与错误码），或引入 springdoc 并验证 swagger 可访问；详设 §3.2 是设计契约不是 API 文档。

### L-M01-011：API 探针不可替代浏览器 E2E，密钥契约两端都要测

**教训**：M01 的 P4/P6"真实验证"以 API 探针（Python 侧独立实现 RSA 加密）替代浏览器 E2E，两个前端缺陷全部漏网直到人工首验——① `rsa.ts` DER 解析把外层 SEQUENCE/BIT STRING 整体跳过导致解析越界，浏览器登录必崩（后端契约测试与 API 探针全绿，探不到前端半边）；② 根路由 `redirect: '/dashboard'` 与 M01 菜单种子（无 dashboard）不一致，登录后 404。同时 `e2e/smoke.spec.ts` 仍是老脚手架断言（"登 录"按钮/仪表盘/用户管理），未随迁移更新，形同虚设却充当了覆盖证据。

**规约（已固化 v3.27.6）**：
- **定位澄清**：P6 测试是**交付把关**而非缺陷发现——缺陷发现在 P3/P3b/P4；P6 只回答"功能已实现且真实可运行"，真实运行通过才可声明交付。**P6 测出的问题必须修复→重跑测试→循环直到测不出问题（FAIL=0）**，禁止带病交付、禁止降断言换绿灯；每轮按铁律 8 重跑同一 Gate 并保留各轮证据，同时回溯收紧漏过的上游 Gate。
- `phases/06e-浏览器E2E.md` 增加门禁口径：E2E 必须是真实浏览器×真实前端×真实后端×真实密钥实跑，API 探针/MockMvc 不可替代；E2E 断言对象随 UI 迁移同步更新，失效 E2E 按 FAIL 计；冒烟最小集含"登录→落点路由可达→菜单与种子一致"。
- `phases/06a-单元测试.md` 新增 L-P6-002：密钥/加密契约两端（前端解析+加密 × 后端解密）必须各有单测，前端单测用真实注入密钥走往返断言。
- `phases/04b-PRD-实现对比.md` 新增跨产物一致性对账：前端静态路由/redirect ↔ 菜单种子、前端契约代码 ↔ 密钥事实源、E2E 断言 ↔ 当前 UI。

### L-M01-012：人工首验 4 缺陷定责——详设留白与实现自创/漏做的典型组合

**教训**：M01 首验暴露 4 缺陷，定责分布——①新建子部门永禁用：实现自创「根节点不可作上级」禁用规则（详设无此规则）叠加初始默认选中根；②点人员无详情：实现漏做 BOP-16 明文触发「组织树选中用户节点」；③新建菜单 admin 不可见：详设 R29 只定义合成口径未定义 admin 对「新建未授权菜单」的语义，实现权限码侧自创 allPermCodes 旁路而菜单侧未做——半套超设计行为造成不一致；④时间显示 ISO 格式：详设 API 字段表用 string 承载时间却未冻结格式标准，实现 5 处各自 `ISO_LOCAL_DATE_TIME`/`toString()`，绕过框架 JacksonConfig 既有约定。

**规约（已固化 v3.27.6）**：
- `phases/02-详细设计.md` 设计必答新增：权限/可见性语义边界（新建对象是否自动可见/自动授权）、字段格式规范（时间字段类型化或冻结 string 格式）。
- `phases/03b-代码审查.md` 业务逻辑清单新增「设计与实现对账」：无自创规则（条件可追溯 R/BOP/DEC）、BOP 伪代码逐条落实、同一决策多处落实必须对称。
- 本缺陷组合的测试逃逸根因另见 L-M01-011（API 探针替代浏览器 E2E）。
