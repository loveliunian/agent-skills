...
### L-M01-013：@Transactional 回滚吞掉失败路径副作用（P0 级，2026-09-21 登录死锁事故）

**教训**：`login()` 标注 @Transactional，密码错误时 `failAndMaybeLock` 写失败计数后抛 BizException——计数随事务回滚**静默丢失**；API 响应（401 AUTH_001）完全符合预期，锁定阶梯与 AFTER_N 验证码策略成为永不触发的死代码。任何只断言响应的测试（含集成测试）都无法发现，事故后用真实 HTTP 链路连错 3 次才暴露。

**规约**：「失败路径上必须跨回滚存活」的状态写入（失败计数、锁定、审计、限流）一律 REQUIRES_NEW 独立事务（TransactionTemplate 或独立 bean 方法，禁止同类自调用），并在独立事务内**重读最新状态**而非复用外层事务实体；P3b 代码审查对 @Transactional 方法内 throw 前的每个写操作逐个判定「回滚后这笔写该不该留」。

### L-M01-014：阈值状态机负路径必须走满阈值，且不得只打豁免账号（三 bug 相互掩护）

**教训**：登录锁定/验证码策略唯一相关用例 a07 只测 admin（isBuiltin=1 完全豁免计数），「普通用户达到阈值」半边零覆盖；后端计数不持久（L-M01-013）使验证码策略永不触发，前端「验证码参数收集了但从未透传」被完全掩护——**三个 bug 相互掩护，happy-path 单测、集成测试、浏览器 E2E 三层同时失明**，全部绿灯直到用户实际触发死锁。

**规约**：凡带阈值的计数/升级/锁定状态机，P5 必须有「走满阈值」负路径用例：失败 N 次 → **回读库内副作用断言**（非仅响应码）→ 升级行为触发 → 恢复路径闭环；普通账号与豁免账号（内置/admin）路径都必须覆盖，豁免账号不得作为该状态机的唯一测试主体；E2E 层至少一条「凭证错误 → 错误提示用户可见」断言（拦截器不得吞掉登录页错误响应）。

### L-M01-015：测试污染防护——失败用例必须自清理，清理语句本身要能写 null

**教训**：a07b 首版断言中断后把 zhangsan 留在 failCount=3，同套件后续用例连锁假失败（login 拿到错误响应 → data null → NPE）；排查中同时发现测试基类 `unlock()` 用 entity+updateById 置空 lock_until——被 MP 默认 NOT_NULL 策略跳过，**清锁定实为 no-op**（L-M01-004 在测试基建中复发：教训已入库但 helper 没落实）。

**规约**：构造脏状态的用例必须 try/finally 自清理，不留跨用例毒化；测试隔离/清理 helper 一律 `LambdaUpdateWrapper.set(field, null)` 显式置空（禁止 entity+updateById 清 null），helper 自身要有证明其有效的用例（如 unlock 后立即可登录）；新增负路径用例须评估「本用例失败时给后续用例留了什么状态」。

### L-M01-016：无默认值环境注入密钥必须配启动脚本；交付前重建产物

**教训**：M01_JWT_SECRET 设计为 env-injected-no-default（TC-SEC-002 安全约束），原进程密钥随启动 shell 丢失 → 换终端裸重启 PlaceholderResolutionException 起不来；临时生成新密钥又使全部已签发 token 失效。同日发现修复后源码未重建 dist——静态部署/preview 会带着旧缺陷「看似已交付实则未交付」。

**规约**：凡 P1 冻结为「无默认值环境注入」的密钥，P7 必须配套启动脚本：首启生成 → 落盘（chmod 600、.gitignore 排除）→ 重启复用同密钥，禁止依赖交互式 shell 的环境变量；P6/P7 交付检查含「产物新鲜度」——源码修复时间必须早于 dist/构建产物时间，或交付前强制重建并核对。

### L-PROC-005：E2E 冷启动预算必须前置声明（超时/等待/串行三要素）

**教训**：m01-base 浏览器 E2E 的 J1/J2 两次超时失败，根因不是功能缺陷而是冷启动预算缺失——dev server 首次编译 + 路由懒加载 + 登录跳转的耗时远超默认断言等待；并行 worker 又放大了首启资源争抢。返工两轮才补齐预算，浪费的 P6 时间全部花在「非缺陷」上。

**规约**：P5 生成 Playwright/E2E 用例时，凡是走「登录 → 受保护页」路径的旅程，必须前置声明冷启动预算三要素：① 导航超时 ≥30s（默认 5~10s 必超）；② 登录后跳转用 `waitForURL` 断言终态而非 sleep/瞬时断言；③ 冷启动用例 `workers=1` 串行（首启资源争抢下并行必然抖动）。三要素缺失的 E2E 用例在 P5 Gate 记 WARN、首次执行超时即回 P5 补预算而不是反复重跑。

### L-PROC-006：评审深度不可用收据仪式替代——时长下限与同输入拦截（已机检化）

**教训**：m01-base P2a 设计评审三轮压缩在 11 分钟内，六角色 started→completed 均 1 秒、六份收据指向同一报告——签名/nonce/两阶段协议全部「合法」，但评审工作实际没有发生（探针未跑、DF 凭空出现）。密码学收据证明的是「谁在何时签了字」，不是「签字前发生了什么」；防漏写的机制不防「演给你看」。

**规约**：① begin 与 complete 之间必须发生真实评审工作，每角色时长 ≥ DEVFLOW_REVIEW_MIN_SECONDS（v3.28.9 起 complete/verify 双侧机检，默认 60s）；② 输入 SHA 未变的整轮重评被拒绝——复审必须先改产物，确需重审须 --rerun-reason 留痕（v3.28.9 机检）；③ 编排器自查：DF 数量与评审时长明显不成比例（如 4 个 DF 对应 1 秒时长）即为仪式化信号，宁可复审不可放行。收据体系证明「流程走了」，探针与 DF 证据链才证明「评审发生了」——两者缺一不可。

## L-详设-图表体系（v3.29.1 起，v3.29.3 扩展，m01-base）

**教训**：
1. 详设的 Mermaid 图由 LLM 会话内手写时逐轮不同（参与者命名/箭头文本/选例），且为凑
   `SEQ>=WHEN` 门禁会产生零信息增量的填充图。
2. 多轮补丁后生成器会累积**多个出图点**（集中式循环+选例残留+节内正确版并存），
   造成图数量翻倍、位置错乱——删除正则若以"其后紧跟 X"定位，易因中间夹层而不命中。
3. 生成器重写全文后**跳过 df_pipeline design 渲染步**直接跑 s2，会因 df:begin 块
   内容回退触发大面积假失败（实测 246 处），补渲染即归零。
4. 详设正文/图出现具体数据库产品名（如 H2）会使设计绑定方言，损害可迁移性。

**对策**：
1. 归属与数量在 P2 前冻结：时序图按 BOP 一操作一张置于专属小节；结构图按七类清单登记——
   状态机→§2.3 状态转移表后、核心机制模型→§4、决策链→对应 §5.x 节首、对象生命周期→所属
   BOP 小节、页面导航→§7.1（从 PAGES 数据派生）、ER→§2.1、§6.0 仅总清单声明不承载图。
2. 生成器**一类图只允许一个出图点**；修改后以「计数按节分布」核验（每 BOP 恰 1 张、
   规则图不越界），并以两次重渲染 SHA 一致作确定性回归；全部图禁止会话内手写。
3. 固定流水线顺序不可省步：gen → df_pipeline design → s2 gate。
4. 数据库中立表述：详设正文与图统一 DB 泛称（图参与者 `participant DB`；正文
   "DB 文件模式/方言由部署配置"），具体方言归《技术选型》《数据库设计决策》与部署配置。
