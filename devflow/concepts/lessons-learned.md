...
**七问题清单**：①表无编号（2.2.N）且表索引锚点非具体表；②接口概览含"概览锚点"冗余列、详细定义未前置；③§3.2 标题塞方法/路径/权限；④业务操作表锚点列在末位；⑤操作小节编号缺 6.2.x；⑥§7.3 页组未覆盖全部页面；⑦追溯矩阵页面/接口/数据三列写通用锚点，与上文一一对应断裂。
...
8. 三处引导已升级为硬校验（check_design_doc_specificity）：§7.3.N 小节数 < 页面数 FAIL、规则锚点单一化 FAIL、§3.2 详细定义小节缺「> 说明：METHOD 路径｜权限」首行 FAIL——负向用例三条均已回归验证。
