...
> **来源**：从 `SKILL.md` 抽出，避免 SKILL.md 超过 500 词限制。
...
| 用户说 | agent 必做 | 脚本 |
|--------|-----------|------|
| `开始实施`、`编码`、`搭建前后端`、`补代码`、`M-01` | **前置拦截（v3.9.5）**：① `docs/requirements/<feature>-acceptance-criteria.md` 存在且 `s0_acceptance_gate.sh` PASS；② `docs/detailed-design/<feature>-design.md` 存在且 `s2_design_coverage_gate.sh` PASS。任一缺失 → 输出 BLOCKED 状态卡片并建议先走 `/spec`。全部通过 → 进入 P3 编码，每次写完代码立即校验 | 拦截：`bash scripts/s0_acceptance_gate.sh <feature>` + `bash scripts/s2_design_coverage_gate.sh ...`；编码期：`bash scripts/build-watchdog.sh check <feature>` |
...
- ❌ 用户说"开始实施" → agent 收到 PRD 就直接编码、不查 `docs/requirements/` 产物 → code0817 型失败（零文档零测试，流程执行率 5%）
