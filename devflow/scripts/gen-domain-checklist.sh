...

{
  ...
  if [ "$STAGE" = "design" ]; then
    ...
    if [ -f "scripts/check_design_doc_quality.py" ]; then
      ...
      echo "- [ ] §7.2 关键页面交互设计消费全部接口节（页组内列调用接口）；数字交叉引用全部有效（证据：核对方式）"
      echo "- [ ] 页面清单完整性三向对账：菜单 seed × client.journeys × UI 验收点——每个可见菜单有页面归属；PRD 称'XX页'但设计为页内区域/抽屉的已写明判定理由（证据：三份清单交点/说明）"
