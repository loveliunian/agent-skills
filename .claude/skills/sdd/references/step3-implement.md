...
- **闸门批量化**(并行在途 ≥2 时的默认节奏,治"编排者瓶颈"):完成通知到了**先攒着**,不收一个处理一个——攒到无新通知或满 6 个,一批跑完各自的 `--stage deliver` + 一批派出各自评审代理。批量闸的判据与单个完全一致,一寸不让;批量内任何一条红只回修该 feature,不拖累同批其他 feature 的评审并行推进。派发实现仍就绪即派,批量化只作用于**交付闸与评审派发**这两个主会话串行段
...
5. **占位销账联动**(复盘 2026-09-20,先例:人员列表占位在数据源交付后漏接):某 feature 交付会解锁 debt-register 中"销账条件"标注的条目时(如交付 usr_user 表解锁"待用户数据源"占位),**派发该 feature 的同时必须派销账工单**(受限边界内的目标模块最小接线,或并入目标模块下一个工单)——占位债务不允许悬空等待
...
- **测试锚点落地自查**(复盘 2026-09-20):spec 规则/验收步骤引用的每个 testId,实现必须落成对应控件的 `data-testid="testId"`,值与详设逐字符一致,禁止另起名/漏加/加在错误层级(加在实际可操作控件根元素上,不加包装容器);自查时对照详设「表单控件/可操作控件」表逐行核对,漏一个即不过自查

## 自查过关

1. `--stage deliver <F编号>` 全绿
2. **隔离复跑铁律**(并行在途时生效):并行有其他 feature 在途时,**全量 `mvn test` 的红不算本 feature 的证据**(可能是他人在途构建噪声:target/ 竞态、jar 写坏、F009 首轮 46 Errors 实为并行 F019 的构建噪声);只认隔离副本复跑——独立 H2 数据目录+独立端口,只测本 feature 涉及模块。绿了才算数,把"共享构建面的红"与"本 feature 的红"分开归因
3. 对照 acceptances 逐条走查补进 impl 日志
4. `--stage set-status <F编号> --status 已交付 --by <谁>`
...
交付呈报时,实现方申报的每一条"归收尾/归后续/遗留问题"**必须当场登记**到 `specs/_work/debt-register.md`(列:来源 F 编号/内容/**销账条件(必须写明解锁数据源的 feature 编号,如"待 F007 交付 usr_user")**/目标 feature 或阶段);未登记的遗留视同未申报。step4 开口第一步即汇总本表成核对清单,done 前逐条核销——**禁止让债务散落在 impl 日志里**。
