...
- **横向切优先**(治依赖链串行,决定并行度上限):能做成"都依赖公共前置、彼此独立"的**扇形结构**,就不要 F2→F3→F4 的**链式结构**——并行层厚度=同一层里独立 feature 个数,关键路径越长可并行越少。拆完自检:同一层内是否有本可拉平的依赖?链式超过 2 层的,逐环问"这个依赖是真数据依赖还是顺手写的"
- **服务/模块归属对账**(详设基线 §5 服务规划):详设 twin 的 feature 带 `service`/`module`、表带 `ownerService` 时,清单逐条比对,不一致登记 Q——sdd 只对账不裁决,归属决定权在详设;详设没有这些字段(老详设)则降级单服务模式照常跑。发现"现有服务装不下、需新建服务"而详设未裁决的,高阻塞 Q 停
- **依赖深度呈报**:清单确认时附 max 依赖链长度与各层 feature 数(如"链深 3:首批 4 个/二层 6 个/三层 2 个"),供用户判断并行度是否够;链深突增要有理由
