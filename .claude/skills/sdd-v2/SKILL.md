...
6. **并行分级**:派发节奏与冲突契约以 step3「事件驱动派发」为唯一口径(ready 探测即就即派);并行**数量**按候选 feature 两两比对 `tables[]` 交集与改动目录定级——无交集且目录不重叠可 3~4 个并行;目录重叠最多 2 个;共享表串行。worktree 并行按 `references/parallel-worktrees.md` 纪律执行(common 先串行 install 一次、副本禁 install/禁起服务/禁连库)。
