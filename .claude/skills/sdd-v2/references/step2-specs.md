...
- **页面交互要点回读**:spec 生成时必须逐条回读其 coversAcceptances 对应详设页的「关键页面交互设计/表格列/表单控件」节,每条交互要点(如"默认展开根节点""入口仅在 X 时可用")要么落进本 spec 的规则或验收步骤,要么在解释性决定清单显式声明归属 feature——禁止把页面交互章节当"前端实现细节"整片跳过(案例:casebook#首屏链路断裂)
