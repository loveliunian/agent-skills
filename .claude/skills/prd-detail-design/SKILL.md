...
[references/system-baseline.md](references/system-baseline.md) 是 skill 的**内置技术上下文**（提炼自系统详细设计总文档）：技术栈（Vue3+TS+Element Plus / Java17+SpringBoot3+MyBatis-Plus+Flyway+H2）、全局接口规范（/api/** 路由动词语义、统一响应结构、全局 bizCode 错误码表、DTO 命名）、全局规则底座（GR02 时间口径/GR03 逻辑删除/GR06 数据所有权/AR 审计等）、Flyway 迁移与公共列约定、**服务规划（微服务为默认架构形态；服务花名册/归属裁决/表所有权/跨服务依赖契约——架构归属在详设定稿，下游 sdd 只对账不裁决）**。约束：
...
5. AI 语义预检补充，合并为 `prd_issues.json`（issues.schema.json 契约）。
   ...
   **深扫硬规则（先例：归档版漏检——语义预检只登记 PRD 自标疑点，13 处深层矛盾被详设生成时静默选边消化，`meta.decisions` 为空）**：
   - 检查单位数 >8（G6 结构报告的单位全集）时，**必须分域派 general-purpose subagent 逐单位通读**（每代理负责一个功能域），不许主会话抽样或凭一处通读代替；
   - **每个检查单位必须有结论记录**：发现问题登记 issue；无问题也要落"通读+无发现+通读范围"一条——分母=scan 结构报告的单位全集，`validate_issues.py --scan <prd_scan.json>` 按 `meta.scanCoverage` 对账，**缺一即红**；
   - **跨域一致性专项（合并时必查）**：拿附录术语/状态索引、全局枚举与各域规则求交集，凡"甲域口径覆盖了乙域明确排除的对象"类矛盾逐条登记（先例：回收站策略生效范围含岗位/角色，而状态索引明文岗位/角色直接删除不进回收站——单域代理各看一半看不见，合并交叉才撞出来）；
   - 通用层/模板层候选逐条表态留档（`scan-dispositions.md`），误报给理由，禁止静默忽略。
...
6. 跑 `validate_issues.py --input design/ch-07/prd_issues.json --scan design/ch-07/prd_scan.json` 自检，渲染 `render_issues.py` 出 `design/ch-07/prd_issues.md`，**交付裁决**。

### 阶段二：裁决回填 → 生成详设（后转）

1. 裁决人在 `prd_issues.json` 逐条落 `status=resolved`（`resolution` 写明口径：按哪个候选、明确什么）或 `wontfix`（理由）；非 blocking 的 open 问题可保留；
   **裁决继承（重跑必做，先例：二次生成时已裁决项被重摊给用户）**：运行目录或归档存在同源 PRD 的历史裁决（USER-DECISIONS 文件或旧 issues 的 resolved 条目）时，先逐条转录继承——status=resolved、resolution 原文、decidedBy 注明"转录继承自 YYYY-MM-DD 裁决"——**已裁决事项不得再次提交裁决**；只有新增发现的问题才进裁决队列；
...
3. 一键硬门流水：
   ...
   python scripts/run_design.py --input design/ch-07/detail_design.json --issues design/ch-07/prd_issues.json \
       ...
       --scan design/ch-07/prd_scan.json
   ```
...
5. **frontend 四段**：pages（页面/模块/路由/组件/类型/权限）、interactions（布局→行操作→三态→并发/错误约定，逐条溯源；**有表格的页带 columns 明细**（字段/列标题/渲染说明），**有表单的页带 formFields 明细**（字段/标签/控件类型/校验提示语/候选来源/**testId 测试锚点**），**有非表单可操作控件（按钮/行操作/搜索/上传等）的页带 controls 明细**（控件名/testId/位置说明））、dialogs（场景→组件→接口→确认流）、**contracts 全局前端契约**（路由守卫/Pinia 划分/API 封装与 bizCode→行为映射/权限方式/复用组件清单，bizCode 优先引用基线通用码；**必含「客户端状态持久化口径」小节**——登录令牌的存储位置（localStorage/内存/cookie）、页面刷新与重开浏览器后的登录态行为、多标签页同步、有效期判定方（前端时钟/服务端 401 惰性）；**必含「测试锚点」全局约定**——所有用户可操作控件统一落 `data-testid`，值 = 详设 formFields/controls 里登记的 testId，详设定名、实现照抄、测试以此精确定位，禁止实现期另起名或漏加；含登录/会话/锁定类模块的，验收点必须成对出现「刷新保持登录态」+「过期/销毁后失效」各至少一条）；要能指导前端直接开工。
