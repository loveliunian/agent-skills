        ...
        if fe.get("interactions"):
            ...
            for it in fe["interactions"]:
                ...
                if it.get("formFields"):
                    ...
                    w(f"  **表单控件（{it['page']}）：**")
                    w("")
                    w("  | 字段 | 标签 | 控件 | 校验与提示 | 候选来源 | testId |")
                    w("  | --- | --- | --- | --- | --- | --- |")
                    for f in it["formFields"]:
                        w(f"  | {_esc(f['field'])} | {_esc(f['label'])} | {_esc(f['control'])} | {_esc(f['rule'])} | {_esc(f['source'])} | `{_esc(f['testId'])}` |")
                if it.get("controls"):
                    w("")
                    w(f"  **可操作控件（{it['page']}）：**")
                    w("")
                    ...
                    for ctl in it["controls"]:
                        w(f"  | {_esc(ctl['name'])} | `{_esc(ctl['testId'])}` | {_esc(ctl.get('note', '—'))} |")
            w("")
        if fe.get("dialogs"):
            w("### 3.3 弹窗/抽屉映射表")
            w("")
