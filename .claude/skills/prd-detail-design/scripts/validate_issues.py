...
def main():
    ...
    ap.add_argument("--scan",
                    help="通用预检产物 prd_scan.json：启用 I7 深扫覆盖对账（分母=其结构报告的检查单位全集）")
    args = ap.parse_args()
    ...
    # I7 深扫覆盖对账（先例：归档版漏检——语义预检只登记 PRD 自标疑点，13 处深层矛盾
    # 未扫出、被详设静默选边消化，meta.decisions 为空。本检查把"每个检查单位被读过、
    # 有结论"从模型自觉变成分母对账：结论记录的分母必须 = scan 结构报告的检查单位全集）。
    if args.scan:
        scan = json.loads(Path(args.scan).read_text(encoding="utf-8"))
        units = [u.get("id") for u in (scan.get("structure") or {}).get("units", []) if u.get("id")]
        cov = data.get("meta", {}).get("scanCoverage") or {}
        cov_ids = set(cov if isinstance(cov, list) else cov.keys())
        missing = [u for u in units if u not in cov_ids]
        extra = sorted(cov_ids - set(units))
        if not units:
            warns.append("I7: scan 文件无检查单位清单（structure.units 为空），跳过覆盖对账")
        elif missing:
            errors.append(
                f"I7 深扫覆盖不完整：{len(missing)}/{len(units)} 个检查单位无结论记录——"
                f"缺 {missing}。语义预检必须逐单位通读：发现问题登记 issue，无问题也要在"
                f"meta.scanCoverage 留『通读+无发现』记录，缺一即红（防漏检静默收工）"
            )
        if extra:
            warns.append(f"I7: scanCoverage 含 scan 清单之外的单位 {extra}（疑似单位 id 拼写错误）")
